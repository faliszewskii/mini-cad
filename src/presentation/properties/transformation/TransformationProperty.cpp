...
TransformationProperty::TransformationProperty()
        : position(), orientation(glm::quat(1, 0, 0, 0)), scale(glm::vec3(1.0f)) {}

TransformationProperty::TransformationProperty(Bindable<glm::vec3> position, Bindable<glm::quat> orientation,
...
glm::mat4 TransformationProperty::getTransformation() {
    return glm::translate(glm::mat4(1.0f), position.get()) * glm::scale(glm::mat4(1.0f), scale.get()) *
           glm::mat4_cast(orientation.get());
}
